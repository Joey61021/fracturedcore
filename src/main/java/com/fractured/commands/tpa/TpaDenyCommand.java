...
public final class TpaDenyCommand
{
    ...
    public static boolean deny(final CommandSender sender, final Command cmd, final String label, final String[] args)
    {
        ...
        for (TpaRequest request : TpaManager.getRequests())
        {
            if (request.getRequested().equals(player.getUniqueId()))
            {
                ...
                target.sendMessage(FracturedCore.getMessages().get(Messages.COMMAND_TPA_DENIED));
                return true;
            }
        }
        return true;
